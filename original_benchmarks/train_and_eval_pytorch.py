...
def main(data_flag, output_root, num_epochs, gpu_ids, batch_size, size, download, model_flag, resize, as_rgb, model_path, run):
    ...
    for epoch in trange(num_epochs):        
        ...
        # Check for improvement and update best model/epoch
        if cur_auc - best_auc > min_delta:
            best_epoch = epoch
            best_auc = cur_auc
            best_model = deepcopy(model)
            print(f'\nEpoch {epoch}: Found new best val_auc: {best_auc:.5f}')
            no_improvement_count = 0 # Reset counter
        else:
            no_improvement_count += 1
            print(f'\nEpoch {epoch}: No significant improvement for {no_improvement_count} epochs.')
